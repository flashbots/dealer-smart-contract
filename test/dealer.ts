...
describe("dealer tests", async function () { 
    it("zero tests", async function() {
        ...
        const data = fs.readFileSync('./test/data/case0/orders.json', 'utf-8');
        const orders = buildOrders(data, tokensDict);

        // get signatures
        const pKeys = privateKeys.slice(0, 1);
        const messages: string[] = encodeOrders(orders);
        const signatures = signMessages(pKeys, messages);
        
        // form data from filler
        const fillerDataString = fs.readFileSync('./test/data/case0/fillerData.json', 'utf-8');
        const [transfersFromInfo, transactions] = getFillerInput(fillerDataString, usersDict);
